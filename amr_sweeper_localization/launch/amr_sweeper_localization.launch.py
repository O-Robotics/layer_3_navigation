...
def _launch_fusioncore(context, *args, **kwargs):
    ...
    use_visual_odometry = (
        LaunchConfiguration('use_visual_odometry').perform(context).lower() == 'true'
    )
    ...
    fusion_overrides = {
        ...
        'encoder.topic': _primary_topic_or_disabled(
            use_encoder,
            parameters.get('encoder.topic', 'drive_controller/odom'),
            '_encoder_disabled',
        ),
        'encoder2.topic': _topic_if_enabled(
            use_visual_odometry, parameters.get('encoder2.topic', 'visual_odometry/odom')),
        'gnss.use_gps_fix': use_gnss and parameters.get('gnss.use_gps_fix', False),
        ...
        'gnss.heading_topic': _topic_if_enabled(
            use_gnss, parameters.get('gnss.heading_topic', '')
        ),
        'gnss.azimuth_topic': _topic_if_enabled(
            use_gnss, parameters.get('gnss.azimuth_topic', '')
        ),
        'reference.use_first_fix': use_gnss and parameters.get('reference.use_first_fix', True),
        'init.wait_for_all_sensors': (
            parameters.get('init.wait_for_all_sensors', True) and use_encoder
        ),
        'publish.tf': False,
    }
    ...
    if use_visual_odometry:
        actions.append(visual_odometry_node)
    actions.extend([projection_node, node, configure, activate])
    return actions
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                ...
                default_value='true',
                description=(
                    'If false, relax FusionCore RTK gating and reduce GNSS trust '
                    'to match autonomous fixes.'
                ),
            ),
