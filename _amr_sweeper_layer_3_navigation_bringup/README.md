...
- `amr_sweeper_navigation` selects `default_missions_navigation.launch.py`, `manual_missions_navigation.launch.py`, or `programmed_missions_navigation.launch.py` from the active mission context. Built-in local pattern missions still plan in `odom`, but when mapping is requested they now keep `amr_sweeper_mapping` active so Nav2 can consume `mapping/local_costmap` and `mapping/global_costmap`.
...
- Mission-triggered rosbag recording is handled by `amr_sweeper_mission_executor`, so Teleop and other profiles that skip layer 3 can still capture bags into the mission run's `artifacts/rosbag` folder.
